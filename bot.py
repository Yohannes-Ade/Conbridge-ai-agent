...
# ── Helpers ───────────────────────────────────────────────────────────────────
def fmt(item, unit="ETB"):
    """Format price line. Shows today's price + market range if available."""
    price = item["price"]
    rng   = item.get("range")
    if rng:
        return f"  • {item['name']} — <b>{price} {unit}</b>  <i>(range: {rng})</i>"
    return f"  • {item['name']} — <b>{price} {unit}</b>"
...
def build_message(cement, steel, aggregates, exchange, fuel) -> str:
    ...
    # Steel
    lines.append("🔩 <b>REINFORCEMENT STEEL (ETB/kg)</b>")
    if steel:
        for item in steel:
            lines.append(fmt(item))
    else:
    ...
    # Exchange rate
    lines.append("💱 <b>EXCHANGE RATE (NBE official)</b>")
    if exchange:
        lines.append(f"  • 1 USD = <b>{exchange['usd_etb']} ETB</b>")
        lines.append(f"  • 1 EUR = <b>{exchange['eur_etb']} ETB</b>")
    else:
        lines.append("  • Data unavailable today")
    lines.append("")

    lines.append("─────────────────────────")
    lines.append("📡 <i>Sources: Jiji, ConMaret, Birr Metrics, NBE | Addis Ababa retail</i>")
